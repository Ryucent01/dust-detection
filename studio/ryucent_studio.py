...
class LoginDialog(QWidget):
    ...
    def check_auth(self):
        ...
        try:
            # Check terminal_access collection for the key
            docs = self.db.collection("terminal_access").where("key", "==", access_key).limit(1).get()
            
            if len(docs) > 0 and docs[0].to_dict().get("active", False):
...
class RyucentStudio(QMainWindow):
    def __init__(self):
        ...
        if not os.path.exists(SERVICE_ACCOUNT_KEY):
            QMessageBox.critical(self, "Error", f"{SERVICE_ACCOUNT_KEY} not found!")
            sys.exit(1)
    ...
    def load_data(self, status):
        self.img_list.clear()
        docs = self.db.collection("images").where("status", "==", status).limit(100).stream()
        for doc in docs:
            data = doc.to_dict()
    ...
    def on_item_clicked(self, item):
        ...
        if self.current_mode == "labeled":
            doc = self.db.collection("images").document(doc_id).get()
            data = doc.to_dict()
            # self.canvas.clear_boxes() # Removed, already done above
    ...
    def submit_labels(self):
        ...
        try:
            ...
            self.db.collection("images").document(self.current_doc).update({
                "status": "labeled",
                "labels": labels,
