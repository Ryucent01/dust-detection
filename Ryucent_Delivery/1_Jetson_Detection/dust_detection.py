...
SUPABASE_AVAILABLE = SUPABASE_URL is not None and SUPABASE_KEY is not None
...
FULLSCREEN_MODE = True            # Set to True for fullscreen display on Linux/Jetson
...
class ImageSyncSystem:
    def __init__(self):
        ...
        if SUPABASE_AVAILABLE:
             ...
             print("[WARN] Supabase credentials missing in .env. Sync disabled.")
            ...
            try:
                ...
                if self.supabase:
                    try:
                        ...
                        self.supabase.table(SUPABASE_TABLE).insert({
                            "filename": filename,
                        ...
                        uploaded = True
                        print(f"[SUCCESS] Supabase Upload: {filename}")
                    except Exception as e:
                        print(f"[WARN] Supabase upload failed for {filename}: {e}")
...
class DustDetector:
    def __init__(self):
        ...
        self.converter = None        # Basler Image Converter
    ...
    def init_camera(self):
        ...
        if self.use_basler:
            ...
            try:
                ...
                # --- STABILITY SETTINGS (GigE) ---
                try:
                    # 1. Heartbeat Timeout: Increase to 5s (prevents disconnect during heavy AI load)
                    if hasattr(self.camera, 'GevHeartbeatTimeout'):
                        self.camera.GevHeartbeatTimeout.SetValue(5000)
                    
                    # 2. Packet Resend: Ensure minor network glitches don't kill the stream
                    if hasattr(self.camera, 'GevSCPSPacketResend'):
                        self.camera.GevSCPSPacketResend.SetValue(True)
                        
                    print("[INFO] Basler stability settings applied.")
                except Exception as e:
                    print(f"[WARN] Could not set some Basler Gev parameters: {e}")

                self.camera.StartGrabbing(pylon.GrabStrategy_LatestImageOnly) 
                
                self.converter = pylon.ImageFormatConverter()
                # Check if camera is monochrome or color to set appropriate conversion
                pixel_format = self.camera.GetNodeMap().GetNode("PixelFormat").GetValue()
                if self.camera.GetDeviceInfo().GetModelName().find('monochrome') != -1 or \
                   "Mono" in pixel_format:
                    ...
                    if hasattr(self, 'use_basler') and self.use_basler:
                        try:
                            ...
                            if self.camera.IsGrabbing():
                                ...
                                # Camera exists but not grabbing
                                self.consecutive_errors += 1
                                if self.consecutive_errors >= self.error_threshold:
                                    self.recover_basler()
                                time.sleep(1)
    ...
    def recover_basler(self):
        """
        Attempts to restart the Basler camera connection.
        """
        print("\n" + "!" * 40)
        print("[RECOVERY] Basler connection lost. Attempting auto-restart...")
        print("!" * 40 + "\n")
        ...
        try:
            ...
            self.init_camera()
            self.consecutive_errors = 0
            print("[INFO] Basler recovery sequence complete.")
        except Exception as e:
            print(f"[ERROR] Basler recovery failed: {e}")
            time.sleep(1)
